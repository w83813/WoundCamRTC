    ...
    ImageViewer.prototype = {
        ...
        _init: function () {
            ...
            imageWrap.on('touchstart' + eventSuffix, function (estart) {
                if (!viewer.loaded) return;
				console.log( "estart.originalEvent.touches.length: " + estart.originalEvent.touches.length);
				if ( estart.originalEvent.touches.length > 1 )
				    return;
                ...
                if (viewer.onTouchCallback!=null) {
                    console.info("touchstart touch0.pageX:"+touch0.pageX +", imageWrap.offsetX:"+ imageWrap.offsetX);

                    var z = viewer.zoomValue/100.0;
                    if (imageWrap.offsetX==undefined ||imageWrap.offsetY==undefined)
                        viewer.onTouchCallback(touch0.pageX, touch0.pageY,z);
                    else
                        viewer.onTouchCallback(touch0.pageX + imageWrap.offsetX,touch0.pageY + imageWrap.offsetY,z);
                }
            ...
            imageWrap.on('click' + eventSuffix, function (e) {
                if (viewer.onClickCallback!=null) {
                    console.info("click");
                    var z = viewer.zoomValue/100.0;
                    viewer.onClickCallback(e.offsetX/z,e.offsetY/z,z);
                }
                if (touchtime == 0) {
                    touchtime = Date.now();
                    point = {
