...
def remove_client_from_room(host, room_id, client_id):
  ...
  # Compare and set retry loop.
  while True:
    logging.info('Room has User ' + str(room))
    if room is None:
      logging.warning('remove_client_from_room: Unknown room ' + room_id)
      memcache_client.set(key, room)
...
def remove_other_client_from_room(host, room_id, client_id):
  key = get_memcache_key_for_room(host, room_id)
  memcache_client = memcache.Client()
  retries = 0
  room = memcache_client.gets(key)
  ...
  while True:
    logging.info('Room has User ' + str(room))
    other_client = room.get_other_client_id(client_id)
    if room is None:
      logging.warning('remove_other_client_from_room: Unknown room ' + room_id)
      return {'error': constants.RESPONSE_UNKNOWN_ROOM, 'room_state': ''}
    if not room.has_client(other_client):
      #logging.warning('remove_other_client_from_room: Unknown client ' + other_client + ' for room ' + room_id)
      memcache_client.set(key, room)
      return {'error': constants.RESPONSE_UNKNOWN_CLIENT, 'room_state': ''}
      #if room.get_occupancy() <= 0:
	  #  return {'error': None, 'room_state': str(room)}
    logging.info('Removed client ' + other_client + ' from room ' + room_id)
    room.remove_client(other_client)

    #if memcache_client.cas(key, room, constants.ROOM_MEMCACHE_EXPIRATION_SEC):
    #  logging.info('Removed client %s from room %s, retries=%d' \
    #      %(other_client, room_id, retries))
    #  return {'error': None, 'room_state': str(room)}
    #retries = retries + 1 
...
class LeavePage(webapp2.RequestHandler):
  def post(self, room_id, client_id):
    logging.info('LeavePage User ' + client_id + ' leave Room ' + room_id)
    result = remove_client_from_room(
        self.request.host_url, room_id, client_id)
    if result['error'] is None:
      logging.info('Room ' + room_id + ' has User ' + result['room_state'])

class ClearPage(webapp2.RequestHandler):
  def get(self, room_id, client_id):
    logging.info('ClearPage: ' + room_id + ' , which:' + client_id)
    room = memcache.get(get_memcache_key_for_room(self.request.host_url, room_id))
    if room is not None:
      if room.get_occupancy() >= 1:
        result = remove_other_client_from_room(
            self.request.host_url, room_id, client_id)
        if result['error'] is not None:
          logging.info('Unexpected disconnection')
        logging.info('Room ' + room_id + ' has User ' + result['room_state'])	
    else:	
      logging.info('room is None')
...
app = webapp2.WSGIApplication([
    ...
    ('/clear/([a-zA-Z0-9-_]+)/([a-zA-Z0-9-_]+)', ClearPage),
