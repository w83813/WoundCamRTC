...
package org.itri.woundcamrtc.ocr.decode;
    ...
    public void handleMessage(Message message) {
        switch (message.what) {
            case R.id.auto_focus:
                ...
                if (mState == State.PREVIEW && mActivity.isRequestPreviewFrame) {
                    //CameraManager.get().requestAutoFocus(this, R.id.auto_focus);
                    CameraManager.get().requestPreviewFrame(new Camera.PreviewCallback() {
                        @Override
                        public void onPreviewFrame(byte[] data, Camera camera) {
                            mActivity.processPreviewFrame(data,camera);
                            mActivity.isRequestPreviewFrame = false;
                        }
                    });
                }
                break;
