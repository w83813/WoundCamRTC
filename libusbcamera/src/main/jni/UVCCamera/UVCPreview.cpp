...
//#include "../../../../../../../../../dev/android-ndk-r14b/platforms/android-23/arch-x86/usr/include/unistd.h"
...
int UVCPreview::setPreviewSize(int width, int height, int min_fps, int max_fps, int mode,
    ...
    if ((requestWidth != width) || (requestHeight != height) || (requestMode != mode)) {
        ...
        switch (requestMode) {
            case 3: //UVC_FRAME_FORMAT_YUYV
                result = uvc_get_stream_ctrl_format_size_fps(mDeviceHandle, &ctrl,
                                                             UVC_FRAME_FORMAT_YUYV, requestWidth,
                                                             requestHeight, requestMinFps,
                                                             requestMaxFps);
                break;
            case 4: //UVC_FRAME_FORMAT_UYVY
                result = uvc_get_stream_ctrl_format_size_fps(mDeviceHandle, &ctrl,
                                                             UVC_FRAME_FORMAT_UYVY, requestWidth,
                                                             requestHeight, requestMinFps,
                                                             requestMaxFps);
                break;
            case 6: //UVC_FRAME_FORMAT_RGB
                result = uvc_get_stream_ctrl_format_size_fps(mDeviceHandle, &ctrl,
                                                             UVC_FRAME_FORMAT_RGB, requestWidth,
                                                             requestHeight, requestMinFps,
                                                             requestMaxFps);
                break;
            case 10: //UVC_FRAME_FORMAT_GRAY8
                result = uvc_get_stream_ctrl_format_size(mDeviceHandle, &ctrl,
                                                         UVC_FRAME_FORMAT_GRAY8, requestWidth,
                                                         requestHeight, requestMaxFps);
                break;
            case 11: //UVC_FRAME_FORMAT_Y16
                result = uvc_get_stream_ctrl_format_size(mDeviceHandle, &ctrl, UVC_FRAME_FORMAT_Y16,
                                                         requestWidth, requestHeight,
                                                         requestMaxFps);
                break;
            default:
                result = uvc_get_stream_ctrl_format_size_fps(mDeviceHandle, &ctrl,
                                                             UVC_FRAME_FORMAT_MJPEG, requestWidth,
                                                             requestHeight, requestMinFps,
                                                             requestMaxFps);
        }
...
void UVCPreview::uvc_preview_frame_callback(uvc_frame_t *frame, void *vptr_args) {
    ...
    if (UNLIKELY(
            ((frame->frame_format != UVC_FRAME_FORMAT_MJPEG &&
              frame->frame_format != UVC_FRAME_FORMAT_Y16)
...
//            pthread_cond_wait(&preview_sync, &preview_mutex);
            errtimeout = pthread_cond_timedwait(&preview_sync, &preview_mutex, &ts);
            if(errtimeout) {
                LOGI("Timeout no frame : Restart preview");
                uvc_stream_stop(mDeviceHandle->streams);
                uvc_stream_start(mDeviceHandle->streams, mDeviceHandle->streams->user_cb, mDeviceHandle->streams->user_ptr,0);
                LOGI("Timeout no frame : Done restart preview");
            }
        }
...
int UVCPreview::prepare_preview(uvc_stream_ctrl_t *ctrl) {
    ...
    switch (requestMode) {
        case 3: //UVC_FRAME_FORMAT_YUYV
            ...
            break;
        case 4: //UVC_FRAME_FORMAT_UYVY
            result = uvc_get_stream_ctrl_format_size_fps(mDeviceHandle, ctrl, UVC_FRAME_FORMAT_UYVY,
                                                         requestWidth, requestHeight, requestMinFps,
                                                         requestMaxFps);
            break;
        case 6: //UVC_FRAME_FORMAT_RGB
            result = uvc_get_stream_ctrl_format_size_fps(mDeviceHandle, ctrl, UVC_FRAME_FORMAT_RGB,
                                                         requestWidth, requestHeight, requestMinFps,
                                                         requestMaxFps);
            break;
        case 10: //UVC_FRAME_FORMAT_GRAY8
            result = uvc_get_stream_ctrl_format_size(mDeviceHandle, ctrl, UVC_FRAME_FORMAT_GRAY8,
    ...
    if (LIKELY(!result)) {
        ...
        if (LIKELY(!result)) {
            ...
            LOGI("frameSize=(%d,%d)@%d", frameWidth, frameHeight, requestMode);
            pthread_mutex_lock(&preview_mutex);
            if (LIKELY(mPreviewWindow)) {
                ANativeWindow_setBuffersGeometry(mPreviewWindow,
                                                 frameWidth, frameHeight, previewFormat);
        ...
        int bytes = (!requestMode ? 2 : 4);
        bytes = 4;
        frameBytes = frameWidth * frameHeight * bytes;
...
void UVCPreview::do_preview(uvc_stream_ctrl_t *ctrl) {
	...
	// https://github.com/groupgets/purethermal1-uvc-capture/blob/master/v4l2/uvcdynctrl/ctrl_gen.py
	// https://www.flir.com/globalassets/imported-assets/document/flir-lepton-software-interface-description-document.pdf
	uint32_t LEP_AGC_MODULE_BASE = 0x0100;
	uint32_t LEP_SYS_MODULE_BASE = 0x0200;
	uint32_t LEP_VID_MODULE_BASE = 0x0300;
	uint32_t LEP_OEM_MODULE_BASE = 0x0800;
	uint32_t LEP_RAD_MODULE_BASE = 0x0E00;

	uint8_t VC_CONTROL_XU_LEP_AGC_ID = 3;
	uint8_t VC_CONTROL_XU_LEP_OEM_ID = 4;
	uint8_t VC_CONTROL_XU_LEP_RAD_ID = 5;
	uint8_t VC_CONTROL_XU_LEP_SYS_ID = 6;
	uint8_t VC_CONTROL_XU_LEP_VID_ID = 7;

	uint32_t LEP_CID_SYS_CAM_STATUS = LEP_SYS_MODULE_BASE + 0x0004;
	uint32_t LEP_CID_SYS_CAM_UPTIME = LEP_SYS_MODULE_BASE + 0x000C; // uint32_t test ok
	uint32_t LEP_CID_SYS_FPA_TEMPERATURE_KELVIN = LEP_SYS_MODULE_BASE + 0x0014; // uint16_t test ok


	uint32_t LEP_CID_OEM_POWER_DOWN = LEP_OEM_MODULE_BASE + 0x0000;
	uint32_t LEP_CID_OEM_STANDBY = LEP_OEM_MODULE_BASE + 0x0004;
	uint32_t LEP_CID_OEM_SOFTWARE_VERSION = LEP_OEM_MODULE_BASE + 0x0020; // test ok
	uint32_t LEP_CID_OEM_REBOOT = LEP_OEM_MODULE_BASE + 0x0040;
	...
	if (LIKELY(!result)) {
		...
		if (false) {
			//LEP_CID_OEM_POWER_DOWN
			commandId = LEP_CID_OEM_POWER_DOWN;
			unitId = VC_CONTROL_XU_LEP_OEM_ID;
			ctrlId = ((commandId & 0x00ff) >> 2) + 1;
			ret = uvc_set_ctrl(mDeviceHandle, unitId, ctrlId, &ctrlId, UVC_SET_CUR);
			if (ret < 0) {
				LOGI("uvc_power_down is failed, ret = %d\n", ret);
			}
			LOGI("uvc_power_down: ret = %d\n", ret);;
		}
		...
		if (false) {
			...
			ret = uvc_set_ctrl(mDeviceHandle, unitId, ctrlId, &ctrlId, UVC_SET_CUR);
			if (ret != len) {
				LOGI("uvc_reboot is failed, ret = %d\n", ret);
			}
			LOGI("uvc_reboot: retLen = %d, reqLen = %d, data = %d\n", ret,
				 len, data8);
		}
		...
		if (frameMode == 11) {
			...
			if (false) {
				...
				ret = uvc_set_ctrl(mDeviceHandle, unitId, ctrlId, &ctrlId, UVC_SET_CUR);
				if (ret < 0) {
					LOGI("uvc_power_on is failed, ret = %d\n", ret);
				}
				LOGI("uvc_power_on: ret = %d\n", ret);;
			}
			...
			for (; LIKELY(isRunning());) {
				...
				if (checkFPA++ > 360) {
//                    //LEP_CID_OEM_SOFTWARE_VERSION
//                    commandId = LEP_CID_OEM_SOFTWARE_VERSION;
//                    unitId = VC_CONTROL_XU_LEP_OEM_ID;
//                    ctrlId = ((commandId & 0x00ff) >> 2) + 1;
//
//                    LEP_OEM_SW_VERSION_T dataSwVer;
//                    len = sizeof(dataSwVer);
//                    ret = uvc_get_ctrl(mDeviceHandle, unitId, ctrlId, (void *) &dataSwVer, len,
//                                       UVC_GET_CUR);
//                    if (ret != len) {
//                        LOGI("uvc_get_software is failed, ret = %d\n", ret);
//                    }
//                    LOGI("uvc_get_oem_software_ver: retLen = %d, reqLen = %d, data = %d\n", ret,
//                         len, dataSwVer);
//                    LOGI("uvc_get_oem_software_ver: gpp_major = %d, gpp_minor = %d, gpp_build = %d\n",
//                         dataSwVer.gpp_major, dataSwVer.gpp_minor, dataSwVer.gpp_build);
//                    usleep(100);
//                    //LEP_CID_SYS_CAM_STATUS
//                    commandId = LEP_CID_SYS_CAM_STATUS;
//                    unitId = VC_CONTROL_XU_LEP_SYS_ID;
//                    ctrlId = ((commandId & 0x00ff) >> 2) + 1;
//                    LEP_STATUS_T dataStatus;
//                    len = sizeof(dataStatus);
//                    ret = uvc_get_ctrl(mDeviceHandle, unitId, ctrlId, (void *) &dataStatus, len,
//                                       UVC_GET_CUR);
//                    if (ret != len) {
//                        LOGI("uvc_get_sys_status is failed, ret = %d\n", ret);
//                    }
//                    LOGI("uvc_get_sys_status: retLen = %d, reqLen = %d, data = %d\n", ret, len,
//                         dataStatus);
//                    LOGI("uvc_get_sys_status: camStatus = %d, commandCount = %d\n",
//                         dataStatus.camStatus, dataStatus.commandCount);
//
//                    usleep(100);
//                    //LEP_CID_SYS_CAM_UPTIME
//                    commandId = LEP_CID_SYS_CAM_UPTIME;
//                    unitId = VC_CONTROL_XU_LEP_SYS_ID;
//                    ctrlId = ((commandId & 0x00ff) >> 2) + 1;
//                    len = sizeof(data32);
//                    ret = uvc_get_ctrl(mDeviceHandle, unitId, ctrlId, (void *) &data32, len,
//                                       UVC_GET_CUR);
//                    if (ret != len) {
//                        LOGI("uvc_get_sys_uptime is failed, ret = %d\n", ret);
//                    }
//                    LOGI("uvc_get_sys_uptime: retLen = %d, reqLen = %d, data = %d\n", ret, len,
//                         data32);
//
//                    usleep(100);
					...
					ctrlId = ((commandId & 0x00ff) >> 2) + 1;
					len = sizeof(data16);
					ret = uvc_get_ctrl(mDeviceHandle, unitId, ctrlId, (void *) &data16, len,
									   UVC_GET_CUR);
					if (ret != len) {
						LOGI("uvc_get_fpa_kelvin is failed, ret = %d\n", ret);
					}
					LOGI("uvc_get_fpa_kelvin: retLen = %d, reqLen = %d, data = %d\n", ret, len, data16);
					fpa_kelvin = (double) data16;
					checkFPA = 0;
				}

				// y16 mode
				//LOGI("waitPreviewFrame...b");
				frame = waitPreviewFrame();
				//LOGI("waitPreviewFrame...a");
				if (LIKELY(frame)) {
					if (LIKELY(isRunning()))
						addCaptureFrame(frame);
					//LOGI("waitPreviewFrame...c");
				}
