...
public final class USBMonitor {
    ...
    private static final String TAG = USBMonitor.class.getSimpleName() + ".uvc";
     ...
     */
    private ConcurrentHashMap<UsbDevice, UsbControlBlock> mCtrlBlocks = null;
    private SparseArray<WeakReference<UsbDevice>> mHasPermissions = null;

    private WeakReference<Context> mWeakContext = null;
    private UsbManager mUsbManager = null;
    private OnDeviceConnectListener mOnDeviceConnectListener = null;
    private PendingIntent mPermissionIntent = null;
    private List<DeviceFilter> mDeviceFilters = null;
    ...
    private static Semaphore mStartStopSemaphore = null;
    ...
    public USBMonitor(final Context context, final OnDeviceConnectListener listener) {
        ...
        if (mStartStopSemaphore == null) {
            mStartStopSemaphore = new Semaphore(1);
        }
        try {
//			synchronized (mStartStopSemaphore) {
            if (mStartStopSemaphore.tryAcquire(1, TimeUnit.SECONDS)) {
                if (listener == null)
                    throw new IllegalArgumentException("OnDeviceConnectListener should not null.");
                mCtrlBlocks = new ConcurrentHashMap<UsbDevice, UsbControlBlock>();
                mHasPermissions = new SparseArray<WeakReference<UsbDevice>>();
                mDeviceFilters = new ArrayList<DeviceFilter>();
        ...
        if (!destroyed) {
            ...
            if (mAsyncHandler != null) {
                try {
                    ...
                    if (mAsyncHandler.getLooper() != null) mAsyncHandler.getLooper().quit();
                } catch (final Exception e) {
                    Log.e(TAG, "destroy:", e);
                }
            }
            ...
            mWeakContext.clear();
            mWeakContext = null;
            mHasPermissions = null;
            mDeviceFilters = null;
            try {
                if (mStartStopSemaphore != null) {
//					synchronized (mStartStopSemaphore) {
                    mStartStopSemaphore.release();
//					}
                }
            } catch (Exception e) {
                e.printStackTrace();
        ...
        if (deviceList != null) {
            if ((filters == null) || filters.isEmpty()) {
                ...
                for (final UsbDevice device : deviceList.values()) {
                    Log.d(TAG, "UsbDevice vid:" + device.getVendorId() + ", pid:" + device.getProductId());
                    // match devices
 ...
 */
     ...
     * @throws IllegalStateException
     */
    public List<UsbDevice> getDeviceList(final DeviceFilter filter) throws Exception {
        if (destroyed) throw new IllegalStateException("already destroyed");
        final List<UsbDevice> result = new ArrayList<UsbDevice>();
        try {
            final HashMap<String, UsbDevice> deviceList = mUsbManager.getDeviceList();
...
//		});
    ...
    public static final String getDeviceKeyName(final UsbDevice device, final String serial, final boolean useNewAPI) {
        ...
        if (useNewAPI && com.serenegiant.utils.BuildCheck.isAndroid5()) {
            sb.append("#");
            if (TextUtils.isEmpty(serial)) {
            ...
            sb.append("#");    // API >= 21
            if (com.serenegiant.utils.BuildCheck.isMarshmallow()) {
...
//		if (DEBUG) Log.v(TAG, "getDeviceKeyName:" + sb.toString());
    ...
    public static UsbDeviceInfo updateDeviceInfo(final UsbManager manager, final UsbDevice device, final UsbDeviceInfo _info) {
        ...
        if (device != null) {
            if (com.serenegiant.utils.BuildCheck.isLollipop()) {
...
//			if (DEBUG) {
            if (mConnection != null) {
                ...
                Log.i(TAG, String.format(Locale.US, "UsbControlBlock name=%s,desc=%d,busnum=%d,devnum=%d,rawDesc=", name, desc, busnum, devnum) + rawDesc);
            } else {
